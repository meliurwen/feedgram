...
This is in order to mask the bot as a comon browser with a common operative system and a standard preferred language.
...
def get_url(url):
    ...
    Sends an http ``GET`` request to the issued url and returns a dictionary with the content and the status code of the response.

    Args:
        url (str): The already urlencoded Url with the payload.

    Returns:
        dict: A dictionary containing the response with its `status_code` (int)
